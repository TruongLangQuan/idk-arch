...
Variants {
    ...
    PanelWindow {
        ...
        readonly property bool isLiveWallpaper: {
            const p = bgRoot.wallpaperPath;
            return p === "cmatrix" || p === "blackhole" || p === "atom" || p === "butterfly" || p === "chaos" || p === "nbody" || p === "bitnet" || p === "ascii";
        }

        onWallpaperPathChanged: {
            if (!bgRoot.isLiveWallpaper) {
        ...
        function updateZoomScale() {
            if (bgRoot.isLiveWallpaper) return;
        ...
        Item {
            ...
            Loader {
                ...
                anchors.fill: parent
                active: bgRoot.isLiveWallpaper
                source: {
                    const p = bgRoot.wallpaperPath;
                    if (p === "cmatrix") return "MatrixRain.qml";
                    if (p === "blackhole") return "Blackhole.qml";
                    if (p === "atom") return "Atom.qml";
                    if (p === "butterfly") return "ButterflyEffect.qml";
                    if (p === "chaos") return "ChaosTheory.qml";
                    if (p === "nbody") return "NBodyProblem.qml";
                    if (p === "bitnet") return "BitnetSimulator.qml";
                    if (p === "ascii") return "AsciiWallpaper.qml";
                    return "";
                }
            }
            ...
            StyledImage {
                id: wallpaper
                visible: opacity > 0 && !blurLoader.active && !bgRoot.isLiveWallpaper
                ...
                source: (bgRoot.wallpaperSafetyTriggered || bgRoot.isLiveWallpaper) ? "" : bgRoot.wallpaperPath
